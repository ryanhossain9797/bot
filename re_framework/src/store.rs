...
pub(crate) fn new_generation() -> i64 {
    ...
    NEXT.get_or_init(|| std::sync::atomic::AtomicI64::new(jiff::Timestamp::now().as_microsecond()))
...
pub(crate) mod contract {
    ...
    pub async fn cas_roundtrip_and_conflict(store: &dyn Store) {
        ...
        assert!(matches!(
            store
                .insert("TestMachine", "e1", "\"e1\"", GEN, "\"v0\"", None, &[])
                .await
                .expect("insert"),
            SaveOutcome::Ok
        ));
    ...
    pub async fn outbox_lifecycle_and_dedup(store: &dyn Store) {
        ...
        assert!(matches!(
            store
                .save(&write(0, 0, drafts, Some(token.clone())))
                .await
                .expect("save"),
            SaveOutcome::Ok
        ));
        ...
        let pending = store
            .pending_outbox("TestMachine", "e1")
            .await
            .expect("pending");
        assert_eq!(
            pending
                .iter()
                .map(|r| (r.seq, r.kind, r.payload_json.as_str()))
                .collect::<Vec<_>>(),
            vec![
                (0, RowKind::Act, "\"a1\""),
                (1, RowKind::Construct, "\"a2\"")
            ]
        );
        let far_future = jiff::Timestamp::now().as_millisecond() + 3_600_000;
        assert_eq!(
            store
                .stalled_outbox_senders(far_future, 10, 0)
                .await
                .expect("stalled"),
        ...
        assert!(
            store
                .is_duplicate("TestMachine", "e1", &token)
                .await
                .expect("dup")
        );
        assert!(
        ...
        let regress = CallToken {
            seq: 6,
            ..token.clone()
        };
        assert!(matches!(
            store
                .save(&write(1, 2, Vec::new(), Some(regress)))
                .await
                .expect("save"),
            SaveOutcome::Ok
        ));
        ...
        assert!(matches!(
            store
                .save(&write(2, 2, Vec::new(), Some(next_gen.clone())))
                .await
                .expect("save"),
            SaveOutcome::Ok
        ));
        assert!(
            store
                .is_duplicate("TestMachine", "e1", &next_gen)
                .await
                .expect("dup")
        );
        assert!(
        ...
        store
            .ack_outbox("TestMachine", "e1", GEN + 1, 0)
            .await
            .expect("stale ack");
        store
            .fail_outbox("TestMachine", "e1", GEN + 1, 1, "boom")
            .await
            .expect("stale fail");
        assert_eq!(
            store
        ...
        store
            .ack_outbox("TestMachine", "e1", GEN, 0)
            .await
            .expect("ack");
        store
            .fail_outbox("TestMachine", "e1", GEN, 1, "boom")
            .await
            .expect("fail");
        assert!(
            store
    ...
    pub async fn generation_guards(store: &dyn Store) {
        ...
        let zombie = TransitionWrite {
            generation: GEN + 1,
            ..write(0, 0, Vec::new(), None)
        };
        assert!(matches!(
    ...
    pub async fn timer_deadlines(store: &dyn Store) {
        ...
        assert!(
            due(i64::MAX).await.expect("due").is_empty(),
            "deadline cleared on commit"
        );
    }
}
